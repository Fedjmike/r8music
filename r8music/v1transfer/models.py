...
class ArtistV1Link(models.Model):
    artist = models.OneToOneField(Artist, on_delete=models.CASCADE, related_name="v1_link")
    old_id = models.IntegerField(unique=True)
    
class ReleaseV1Link(models.Model):
    release = models.OneToOneField(Release, on_delete=models.CASCADE, related_name="v1_link")
    old_id = models.IntegerField(unique=True)
    
class TrackV1Link(models.Model):
    track = models.OneToOneField(Track, on_delete=models.CASCADE, related_name="v1_link")
    old_id = models.IntegerField(unique=True)

class ActionV1Link(models.Model):
    action = models.OneToOneField(Action, on_delete=models.CASCADE, related_name="v1_link")
    old_id = models.IntegerField(unique=True)
